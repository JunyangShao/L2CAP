...
int kpb_pre_l2cap_sig_channel(struct kprobe *p, struct pt_regs *regs){
	...
	while(skb_len >= L2CAP_CMD_HDR_SIZE){
		...
		if(cmd->code == L2CAP_CONN_REQ){
			...
			/* All Parameters get */
			printk("P Server ID = 18062550, dstChanID = %d, l2psm = %d, can_putConn=%d", dstChanID, l2psm, can_putConn);
			/* All Parameters get */
		}
			...
			while (chan_buflen >= L2CAP_CONF_OPT_SIZE) {
				...
				if(type == L2CAP_CONF_MTU){
					...
					/* All Parameters get */
					printk("P Server ID = 18062550, srcChanID = %d, mtuVal = %d, can_putConf=%d", srcChanID, mtuVal, can_putConf);
					/* All Parameters get */
					break;
				}
			}
